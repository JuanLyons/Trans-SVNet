...
criterion_phase = nn.CrossEntropyLoss(
    # weight=torch.from_numpy(weights_train).float().to(device)
)
# criterion_phase = nn.CrossEntropyLoss()
...
train_we_use_start_idx_80 = [x for x in range(4)]
val_we_use_start_idx_80 = [x for x in range(4)]
...
for epoch in range(max_epochs):
    ...
    for i in train_we_use_start_idx_80:
        ...
        long_feature = get_long_feature(
            start_index=train_start_vidx[i],
            lfb=g_LFB_train,
            LFB_length=train_num_each_80[i],
        )
        ...
        _, preds_phase = torch.max(
            y_classes[stages - 1].squeeze().transpose(1, 0).data, 1
        )
    ...
    with torch.no_grad():
        for i in val_we_use_start_idx_80:
            ...
            long_feature = get_long_feature(
                start_index=val_start_vidx[i],
                lfb=g_LFB_val,
                LFB_length=val_num_each_80[i],
            )
            ...
            for j in range(
                stages
            ):  ### make the interuption free stronge the more layers.
                p_classes = y_classes[j].squeeze().transpose(1, 0)
                ce_loss = criterion_phase(p_classes, labels_phase)
                clc_loss += ce_loss
            clc_loss = clc_loss / (stages * 1.0)
            ...
            val_acc_each_video.append(
                float(torch.sum(preds_phase == labels_phase.data)) / val_num_each_80[i]
            )
            # TODO
